    ...
    <div>
      ...
      <div className="flex items-center gap-3 mb-4">
        ...
        {['all', 'active', 'lapsed', 'pending'].map(s => (
          <button key={s} onClick={() => setStatusFilter(s)}
            ...
            style={{
              background: statusFilter === s ? '#623491' : '#ede4f5',
              color: statusFilter === s ? '#e8c487' : '#623491',
              fontFamily: 'Georgia, serif',
              border: statusFilter === s ? 'none' : '1.5px solid #d6c8e4',
            }}>
      ...
      <div className="bg-white rounded-[14px] p-5"
        ...
        <table className="w-full">
          ...
          <tbody>
            {filtered.map(p => (
              <tr key={p.id} className="hover:bg-[#fdf8ec] cursor-pointer">
                <td className="py-[11px] px-3 text-[12.5px] font-bold" style={{ borderBottom: '1px solid #f0e8f5', color: '#2d1a47' }}>{p.name}</td>
                <td className="py-[11px] px-3 text-[12.5px]" style={{ borderBottom: '1px solid #f0e8f5', color: '#7a5ea0' }}>{p.email}</td>
                <td className="py-[11px] px-3 text-[11px] font-bold" style={{ borderBottom: '1px solid #f0e8f5', color: '#623491' }}>{p.awinId}</td>
                ...
                <td className="py-[11px] px-3 text-[12px]" style={{ borderBottom: '1px solid #f0e8f5', color: '#7a5ea0' }}>{p.lastClick || '—'}</td>
                <td className="py-[11px] px-3 text-[12.5px] font-bold" style={{ borderBottom: '1px solid #f0e8f5', color: '#b8860b' }}>${p.allTime.toLocaleString()}</td>
                <td className="py-[11px] px-3 text-[12.5px] font-bold" style={{ borderBottom: '1px solid #f0e8f5', color: p.ytd > 0 ? '#2a9d5c' : '#c0392b' }}>${p.ytd.toLocaleString()}</td>
              </tr>
            ))}
          </tbody>
        </table>
